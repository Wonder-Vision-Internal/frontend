...
export default function PackageDetails() {
    ...
    const getContent = async () => {
        try {
            ...
            setTabs(JSON.parse(datas.data.postDetails?.other_details[0]?.other_details));
    ...
    const handlePayment = async ()=>{
        ...
        try {
            let datas = await OpenApi.get(`payment?name=${data.name}&amount=${data.amount}&number=${data.number}&MUID=${data.MUID}&transactionId=${data.transactionId}`);
    ...
    return (
        <div>
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-md-9">
                            ...
                            <h3 className="destination-head mb-0">{content?.postDetails?.title}</h3>
                            <div className="d-flex mb-3">
                                {content && content.postDetails.other_details[0].days && content.postDetails.other_details[0].nights && <span className="day-pack">{content.postDetails.other_details[0].nights} / {content.postDetails.other_details[0].days}</span>}
                                <a className="custome-your-plan" href="/tailormade">Customize Yours</a>
                            </div>
                                    ...
                                    if (item.content) {
                                        return (
                                            <li className="nav-item" role="presentation" key={index}>
                                                <button className={index === 0 ? "nav-link active" : "nav-link"} id={"home-tab-" + index} data-bs-toggle="tab" data-bs-target={"#home-" + index} type="button" role="tab" aria-controls={"home-" + index} aria-selected="true">{Common.ucWord(item.title)}</button>
                                            </li>
                                        )
                                    }
                            ...
                            <div className="tab-content" id="myTabContent">
                                ...
                                {tabs && tabs.map((item, index) => {
                                    if (item.content) {
                                        return (
                                            <div className={index === 0 ? "tab-pane fade active pb-4 pt-4 show" : "tab-pane fade  pb-4 pt-4"} id={"home-" + index} role="tabpanel" aria-labelledby={"home-data-tab-" + index} dangerouslySetInnerHTML={{
                                                __html: item.content,
                                            }}>
                                            </div>
                                        )
            ...
            {content && content?.postDetails?.gaq_details && <TabFaq videoes={videoes ? videoes : []} gaq_details={content?.postDetails?.gaq_details} />}
            ...
            {content && <Stories stories={content?.postDetails?.stories} name={Common.ucWord(content.postDetails.title)} />}
